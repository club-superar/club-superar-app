...
export default async function RedemptionsPage() {
  ...
  return <main className="profile-shell"><header className="topbar"><Link className="brand" href="/">SUPER<span className="brand-dot">.</span><span className="brand-ar">AR</span><small>CLUB</small></Link><Link className="profile-back" href="/perfil">â† Mi perfil</Link></header><section className="profile-heading"><p className="eyebrow cyan">BENEFICIOS</p><h1>Canjear puntos</h1><p>TenÃ©s <strong>{balance} SUPER Puntos</strong>.</p></section><section className="profile-panel"><RedemptionForm balance={balance} minimum={Number(settings.minimum_redemption_points??10)} arsPerPoint={Number(settings.ars_per_point??100)} rewards={rewardsResult.data??[]}/></section></main>;
}
