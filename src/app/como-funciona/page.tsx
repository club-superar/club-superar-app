...
export default async function HowItWorksPage() {
  ...
  return <main className="info-shell">
    ...
    <section className="info-grid">
      ...
      <details><summary><span>✓</span><strong>Verificación</strong></summary><p>Después de comentar o compartir la historia, tocá “Avisar que ya lo hice”. El paso quedará pendiente hasta que SUPER.AR lo compruebe desde Administración. Conservá el código en el comentario y mantené la historia publicada durante 24 horas.</p></details>
    </section>
    <section className="rules-card"><p className="eyebrow cyan">REGLAS DEL SORTEO</p>{draw ? <><h2>Sorteo #{String(draw.edition_number).padStart(3, "0")}: {draw.title}</h2><ul><li>Premio: {draw.prize_name}.</li><li>Se necesita completar todos los requisitos obligatorios.</li><li>La historia debe permanecer publicada durante 24 horas. Si resultás ganador, Administración podrá pedirte una captura del archivo de historias para comprobarla.</li><li>Máximo de {totalChances} chances según la configuración actual.</li><li>Antes de confirmar al ganador, SUPER.AR vuelve a revisar los requisitos.</li>{draw.closes_at && <li>Cierra el {new Intl.DateTimeFormat("es-AR", { dateStyle: "long", timeStyle: "short", timeZone: "America/Argentina/Buenos_Aires" }).format(new Date(draw.closes_at))}.</li>}</ul></> : <><h2>Próxima edición</h2><p>Las fechas, el premio y los límites aparecerán acá cuando se publique el próximo sorteo.</p></>}</section>
