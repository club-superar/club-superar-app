...
export default async function AdminTicketsPage(){
  ...
  const {data}=await admin.from("purchase_tickets").select("id,storage_path,created_at,issuer_cuit,receipt_type,point_of_sale,receipt_number,issued_on,total_amount,cae,cae_expires_on,profiles!inner(instagram_username)").eq("status","pending").order("created_at");
