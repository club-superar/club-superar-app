        ...
        <div className="admin-draw-list">
          ...
          {(draws ?? []).map((draw) => (
            <article className="admin-draw" key={draw.id}>
              ...
              <div className="admin-draw-actions"><span className={`status-pill status-${draw.status}`}>{statusLabels[draw.status] ?? draw.status}</span>{draw.status === "draft" && <form action={openDraw}><input type="hidden" name="drawId" value={draw.id} /><button type="submit">Abrir sorteo</button></form>}</div>
            </article>
          ))}
        </div>
      </section>
