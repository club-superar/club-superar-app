-- Permite que una excepción individual habilitada por Administración pueda
-- usar Canjes durante la etapa previa al lanzamiento general.
...
declare v_settings jsonb; v_cost integer; v_name text; v_ars integer; v_balance bigint; v_reserved bigint; v_code text; v_hash text; v_expiry timestamptz; v_item public.point_redemptions; v_stock integer; v_pending integer; v_has_override boolean;
begin
  select exists(
  ...
  if not coalesce((select (setting_value->>'redemptions_enabled')::boolean from private.app_settings where setting_key='club_features'),false)
    and not v_has_override then
    raise exception 'REDEMPTIONS_NOT_LAUNCHED';
  end if;
